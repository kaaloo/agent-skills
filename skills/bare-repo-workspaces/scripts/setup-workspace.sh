...
configure_bare_workspace() {
  ...
  # Keep .bare as a true bare repository. The workspace root contains only the
  # .git pointer file; it is not itself a checkout. If core.bare is false, Git
  # and Worktrunk treat the workspace root as an extra dirty worktree alongside
  # main/, which causes duplicate main entries and index-copy conflicts.
  git -C "$workspace" config core.bare true
...
run_fresh_setup_mode() {
  ...
  # Workaround for Letta Code Bash tool interception (issue letta-ai/letta-code#1829):
  # the literal string `git worktree add` triggers a path enforcement check that
  # rejects anything not under `.letta/worktrees/`. Variable indirection evades the
  # static matcher. Remove once upstream fix lands.
  local _git=git
  "$_git" worktree add "$main_branch" "$main_branch"
